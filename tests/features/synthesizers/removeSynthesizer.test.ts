...
const fakeData = async () => {
  return [
    await SynthesizerFactory.props({ id: () => 'firstSynth', name: () => 'First Synth' }).build(),
    await SynthesizerFactory.props({ id: () => 'secondSynth', name: () => 'Second Synth' }).build(),
  ]
}
...
test('Displays a success snack if the synthesizer is correctly deleted', async () => {
  ...
  await story("secondSynth", await fakeData())
